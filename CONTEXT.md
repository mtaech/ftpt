...
几何变换，**裁切框**定义输出区域。应用顺序：先裁切（几何）→ 再色调调整（像素）→ 最后缩放显示。

### 裁切框（Crop Box）

归一化矩形 [x1, y1, x2, y2]（0–1，相对全图），定义输出区域。**区别于检测框（BBox）**：BBox 描述鸟在画面中的位置（识别产物），Crop Box 描述输出区域（用户取景决策）；坐标系相同，语义不同。

### 调整视图（Adjustment View）

进入「调整」tab 时的预览上下文：**强制隐藏全部识别叠加层**（检测框、鸟眼角标、pending 识别框），仅显示裁切框。渲染层条件判断实现，不修改 bbox 显隐状态本身。
