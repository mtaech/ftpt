...
进入「调整」tab 时的预览上下文：**强制隐藏全部识别叠加层**（检测框、pending 识别框），预览显示调整后渲染。渲染层条件判断实现，不修改 bbox 显隐状态本身。
...
对一个 Capture 自动判定物种的过程。三段流水线：**检测**（org_det 开放词表）→ **分类**（BioCLIP）→ **名录映射**。
...
在画面中定位主体（org_det 开放词表 19 个生物粗词，花/虫/菌/鸟都能出框），产出**检测框**。检测不到有效框时识别状态为 Unrecognized。
...
分类器输出的结论经**名录库**补充后得到的具体物种。

- 属性：taxon_id（名录库主键，BioCLIP 大多数预测不在本地名录里为 None）、中文名、学名
- 2026-09-22 起鸟眼锐度阶段与人工修正物种已整体删除（见 `docs/adr/0010`），此处不再有锐度分/眼框字段
