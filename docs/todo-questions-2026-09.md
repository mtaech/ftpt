...
### 1. 预览页复制图片（全尺寸）
- 后端命令 copy_image_to_clipboard（crates/photo-tauri/src-tauri/src/lib.rs）：全尺寸 RGBA 写入系统剪贴板。
...
- 引入 tauri-plugin-clipboard-manager（写图片 + 写文本），capability 放行 allow-write-image / allow-write-text。
...
### 4. 默认图片不堆叠分组
- 堆叠模式默认从 ByTime 改为 None（每文件独立）。涉及 photo-config、config.ts、mock.ts、bindings.ts、CONTEXT.md。
- 你的 ~/.config/pt/config.toml 已是 stackMode = None。

## 验证
- 前端 vue-tsc -b --noEmit ✅、npm run build ✅、vitest 85 用例 ✅
- Rust cargo check -p photo-tauri ✅、photo-config 11 用例 ✅
