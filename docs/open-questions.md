...
- 中国包是**硬地理先验**：**已补「家养/外来」名单**（2026-09-23）——`Felis catus` 家猫、
...
### 7. eBird 导出 —— **入口已补（2026-09-22）、门控拍定维持现状（2026-09-23）**
...
- **已定（2026-09-23）：选 ①**——维持「有物种结论」口径，**不做 `ranks` 落库**（见 `docs/todo.md`
  #10 / #11 维持 ⏸）。理由：本功能就是给观鸟用户用的，非鸟记录会在 CSV 里自己暴露，
  而落库要动 schema 与全部读写/同步/迁移路径。真出现第二个识别后端或要「按类群筛选」时再做。
...
| 项 | 状态 |
|---|---|
| `backend` / `asset_version` 落库 | **没做（已拍板维持不做，见 §7）**。ADR 里写了要记，但 recognition 表还没加这两列；当前靠 `config.toml` 里的后端 + 手动重跑 |
| `ranks`（七级分类）持久化 | **没做（同上）**。内存里有，落库丢了；重载后只剩学名与中文名 |
| 世界包 / f16 压缩 / PCA | **已定 2026-09-23：不随包、不做**（见 §2）；自制路径不变（同构目录） |
