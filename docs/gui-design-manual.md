...
| **导入** | 默认 1120 × 720，**右下角把手可拖拽**（尺寸存配置；上限 = 窗口 − 48px） | **仅 × / 取消 / 完成**（遮罩不关；`Esc` 在放大视图里退回网格） | LR 式**一屏三列**：左来源 / 中审阅（缩略图网格 + 分诊条）/ 右目标与选项；三列宽度固定，审阅网格按中列宽度自动决定列数与格子边长；流程见 §10.3 |
...
LR 式**一屏三列**（左来源 / 中审阅 / 右目标与选项），弹窗**不自动关**。

```
① 左·来源：可移动盘列表（点击即扫描）或浏览目录（扫描只 stat 取文件时间，不读 EXIF）
   「仅添加并浏览」= 不搬运，直接把来源目录打开到网格（原「添加」Tab 合并到这里）
② 中·审阅：扫描完成即出缩略图网格（缓存写**配置目录** import_preview/<源哈希>/，不写卡）
