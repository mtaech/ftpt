...
│    │ 可拖宽       │  CompareView / Slideshow /Stats│ 可拖宽    │       │
├────┴─────────────┴───────────────────────────────┴───────────┴───────┤
...
`view ∈ { grid, preview, compare, slideshow, stats }`，另有一个独立的 `mapOverlay: bool`。

| 视图 | 渲染 | 说明 |
|---|---|---|
| `grid` | 网格 + 筛选栏 | 默认态、空态落点 |
| `preview` | 单图 + 胶片条 | 缩放/平移/叠加层 |
| `compare` | 2–4 格并排 | 进入前视图记在 `viewBeforeCompare` |
...
```
Stats → 空态(无目录) → Slideshow → Compare → Preview → Grid
```
...
| grid·preview → compare | `C`（多选 2–4 张；不足 2 张时取当前项所在连拍组前 4 张） |
| compare → 来源视图 | `Esc` / `G`（还原 `viewBeforeCompare`） |
| grid·preview → slideshow | `S`（从当前选中张在筛选序中的位置开始） |
| slideshow → 来源视图 | `Esc` / `G`（还原 `viewBeforeSlideshow`） |
| 任意 → stats | `T`（进入前若在对比先关闭） |
| stats → grid | `Esc` / `G` / "退出"（清空缓存，下次进入重拉） |
| 任意 → 地图 | `M`（全屏浮层，**不改变 view**） |

`G` 是**唯一"切换"语义键**：网格/预览间切换；在对比/幻灯片/统计态则退回网格。
...
**对比模式**：缩放系数相对**各格自己的 fit**，平移按各格显示尺寸占比**归一化**，
一组缩放平移值**作用于全部窗格**（同步对比的必要条件）。
...
评分/色标/旗标/删除**共用同一目标解析**，避免对比/幻灯片模式误伤未显示的项：
...
| 键 | 动作 | 生效范围 |
|---|---|---|
| `←` / `→` | 上一个 / 下一个 | 网格 = 堆叠组间；对比 = 移聚焦格；幻灯片 = 切张（重置计时） |
| `Home` / `End` | 首 / 末 | 堆叠组 |
| `Q` / `E` | 堆叠内上一个 / 下一个成员 | 仅网格态 |
| `G` | 网格 ⇄ 预览 | 对比/幻灯片/统计态 → 退回网格 |
| `C` | 进入对比 | 多选 2–4 张，或当前项所属连拍组前 4 张 |
| `S` | 进入幻灯片 | 从当前选中张开始 |
| `空格` | 幻灯片暂停 / 继续 | 仅幻灯片态 |
| `T` | 统计视图 进入 / 退出 | 全局 |
| `M` | GPS 地图 进入 / 退出 | 全屏浮层 |
...
| 状态 | 表达 |
|---|---|
| 选中（网格 / 对比 / 胶片条） | 2px accent 环（未选中用 2px 透明边框占位，避免布局跳动） |
...
| `master` | 预览母版，长边 2560（含调整参数烘焙） | 预览主图、对比窗格 |
...
- 拖动**实时改内存**，**350ms 去抖**后持久化。
- 切图前把未落盘的一笔写回**旧图**；卸载前强制 flush。
- 预览母版请求时若该图有非中性参数 → 后端烘焙后返回，并推 `thumb:ready` 触发刷新。
- 参数持久化在文件夹数据库的 `adjustments` 表（键为相对路径）。
...
三个图标按钮：左栏显隐、进入预览、识别所选（`B` 同义；识别中或空库禁用）。
...
单行布局在 200px 宽下不可用。滑杆自绘：4px 轨道 + 50% 中点刻度线，白底圆拇指 + accent 描边，hover 放大 1.15。
...
网格 → 方向键/Q/E 定位 → 1-5 评分 / 6-9 色标 / P·X·U 旗标（可 Ctrl/Shift 多选批量）
     ...
     → C 对比 2–4 张（←/→ 移聚焦格，1-5 给聚焦格评分）/ S 幻灯片过片
...
5. 对比模式不可调整窗格划分（固定 2 列 / 2×2），也不支持 > 4 张。
6. 框选识别只画框，尚未接识别动作。

### 实现层缺口

7. 面板宽度双轨（本地存储 vs 配置字段），不会跨设备同步。
8. 导出/导入目标目录互不记忆。
9. 技术分/重复检测的阈值与结果都不落盘，重启后需重算。
