...
调整 tab **七条滑杆**：曝光 **±3.0 EV（步进 0.3）** / 对比度 / 饱和度 / 阴影 / 高光 / 色温 / 色调
（后六个各 ±100）。曝光范围与步进 2026-09-24 由 ±2.0 / 0.05 调整（用户拍板「太保守」）；
键盘微调「[」「]」= ±0.3 EV、Shift = ±0.9 EV，双击标签归零。
...
| 框选实时框 | `Shift` + 拖拽 | 工具条「框选」toggle 开启、**或按住 Shift** 后左键拖拽，warning 橙实框 + 12% 填充（松开即识别） |
| 框选提示 | — | 图片视口左上角常驻文字胶囊：未按 Shift = 灰字「按住 Shift + 左键拖拽，框选识别补充主体」；按住 Shift = warning 字「已按住 Shift：左键拖拽即可框选识别」；toggle 开启 = 「框选识别已开启：左键拖拽画框，松开后识别」；**拖拽中** = 「松开左键即框选识别 · 按 Esc 取消」 |
...
**框选**（漏检补充）：两种进入方式——工具条「框选」toggle 进入/退出，或**按住 Shift + 左键拖拽**
（快捷键，不改 toggle 状态，工具条按钮 tooltip 里写明）；进入后左键拖拽画框（warning 橙实框，
实时钳制在图片范围内），**松开即识别**——跳过 YOLO 检测直接对该归一化框（0–1）裁剪分类
（引擎 `Recognizer::recognize_region`）；面积过小的框视为误触不触发。识别结果作为**新主体追加**
进该照片已有的 subjects（不覆盖检测器已识别的主体），落 `folder_db` + 全局索引。
**Shift 态提示**：图片视口左上角常驻文字胶囊（常驻文案就写明 Shift 快捷键，按住 Shift 换成
warning 字的「已按住 Shift」），同时光标切成十字；Shift 态既从平台的修饰符变更事件更新，
也从鼠标事件自带的修饰符兜底（无 WM / 无键盘焦点的环境拿不到前者）。
拖拽模式在**左键按下那一刻**定死，鼠标松开时只看有没有框选起点——否则「Shift 画框、松手前先放掉
Shift」会掉进平移分支，框画了却不识别。双击缩放与滚轮缩放不受影响；非框选模式下拖拽仍是平移。

**取消**（2026-09-24 补：用户报过「Shift 框选不能取消」）：拖拽中按 **Esc** 取消这次框
（清掉起点与框，松手不会触发识别，状态栏提示「已取消框选」）；已经画出的框按 **Esc** 清掉
（只清叠加层，不动识别结果）；拖拽中**右键**同样取消（此时不弹右键菜单）。
Esc 的优先级链见 §5.4：识别进行中 → 框选状态 → 幻灯片/统计/预览。
...
3. **识别**：状态 chip（已识别绿 / 待复核橙 / 未检测到灰）+ 物种名 + 相似度
   （BioCLIP 相似度 = 余弦 ×100，跨物种不可比，所以不叫置信度；同时附 top1−top2 间隔）
   + 失败阶段中文 + 最接近候选（后两项尚未接线）
   + 多主体：>1 个主体时主主体在标题行、下方列「另有 N 个主体」（展示名 + 相似度）
   + 动作行 `重新识别` / `显示或隐藏检测框`。
...
- **高度按内容给**（单项 28px + 上下内边距 16px）：放得下就不滚动；只有窗口装不下才限高
  （视口高 − 8px，最少留 120px）并滚动。照片菜单 10 项 = 296px，760px 窗口里**一次看全**——
  2026-09-24 修（用户报「右键菜单太矮」：旧规格是固定 240px 上限，10 项被砍掉两项还得滚）。
...
| **导入** | 默认 1120 × 720，**右下角把手可拖拽**（尺寸存配置；上限 = 窗口 − 48px） | **仅 × / 取消 / 完成**（遮罩不关；`Esc` 在放大视图里退回网格） | **顶部来源条 + 两列**：来源条是标题下方**一行约 40px**（可移动盘 chip / 当前来源路径 / 浏览·重新扫描·仅添加并浏览三个按钮）/ 左列审阅（缩略图网格 + 分诊条）/ 右列目标与选项；只有右列宽度固定，审阅网格按剩余宽度自动决定列数与格子边长；流程见 §10.3 |
| **导出** | 宽 512，高 ≤ 85vh | 遮罩 / `Esc` / × | 预设下拉（新建/保存/删除）+ 长边 + JPEG 质量滑杆 + 命名模板（实时预览第一张）+ 目标目录 |
| **重复照片** | 高 560，宽 760 | 遮罩 / `Esc` / × | 阈值档位（6/8/10/12/16，默认 10）+ 检测按钮 + 汇总行（组数/多余张数/阈值/时间）+ 分组卡片（首张带「保留」徽标）+ 两个动作：「其余标 Rejected」与「保留首张，其余移入回收站」（二次确认） |
...
用户 → 顶栏刷新(F5) / 左栏「导入照片…」/ 收藏·最近卡片单击 / 空态"打开目录"
...
### 10.3 导入（SD 卡 / CFexpress / 外接盘）

**顶部来源条 + 两列**（审阅 / 目标与选项），弹窗**不自动关**。

来源是一次性决定（选完就去审阅），所以它既不占竖向栏位也不做大卡片：原先左栏竖排三张卡片占
240px 宽、卡片下方大半留白、审阅网格少排约两列；现在压成标题下方的**一行**——磁盘 chip、当前来源
路径、三个按钮，说明性文字只留在按钮 tooltip 里（2026-09-25 用户两轮反馈：先「左边的空间合并到
最顶上」，再「搞那么大干啥，缩小啊。文字精简」）。

```
① 顶·来源条（一行）：可移动盘 chip（点击即扫描）/ 当前来源路径（中间占满、太长省略）/
   浏览 · 重新扫描 · 仅添加并浏览。三个小标题与说明句都不在界面上：说明句收进 tooltip
   （「仅添加并浏览」= 不搬运，直接把来源目录打开到网格；原「添加」Tab 合并到这里）
   扫描只 stat 取文件时间，不读 EXIF
② 主·审阅：扫描完成即出缩略图网格（缓存写**配置目录** import_preview/<源哈希>/，不写卡）
   - 分诊条：新 / 已导入 / 冲突 / 源内重复 / 已过滤（+ 该源的「账本去重生效」状态）
   - 类别过滤：照片 / RAW / 视频 / 跳过与 RAW 同名的 JPEG（过滤同时作用于网格与计划）
   - 勾选：点格子切换，或 全选 / 全不选 / 反选；格子上的眼睛角标 = 放大看单张（Esc 退回）
③ 右·目标与选项：目标根目录 + 子目录模式（不建 / YYYY-MM-DD / YYYY/MM/DD / YYYYMMDD）
   + 重命名模板（**实时示例**，与引擎同一套渲染）+ 冲突策略（跳过 / 自动改名 / 覆盖）
   + 导入方式（复制 / 移动）+ 文件处理（复制后校验大小、完成后弹出源盘——仅 Linux）
④ 计划**实时重算**：任何选项/勾选变化 → 300ms 去抖后重算，**没有「生成计划」按钮**
   三层去重：源卷账本（Linux by-uuid / Windows 卷序列号）→ 断点日志 → 目标存在性 + 计划内冲突
   计划明细按类别分组列出每个文件与原因（不截断成 20 条）
⑤ 开始导入：若计划恰好过期（刚改完就点），先重算再自动续跑，不需要用户多点一次
   进度条 + 「取消导入」（每文件开始前检查一次，已搬运保留、剩余不动，粒度一个文件）
   复制后回写源 mtime；同 stem 的 .xmp sidecar 随行；Move+校验走「复制 → 校验 → 删源」
⑥ 结果常驻右栏：汇总 + 失败逐条（路径 + 中文原因）+ 警告逐条；「重试失败项」按失败源重跑；
   全部成功时写账本、丢弃断点日志、可选弹出源盘，并顺手重扫结果目录；点「完成」关窗
外部入口：命令行 `--import <挂载点>` / `--import=<挂载点>` → 打开弹窗、预选源、立即扫描
配方记忆：上次用过的目标/子目录/命名/方式/过滤/策略/校验/弹出整套写进 AppConfig.import_recipe，
          下次打开弹窗自动套用（插卡后基本一键）
断点续传：<目标根>/.pt/import-journal.jsonl 逐条 flush；中断或失败时保留，
          下次对同一目标目录导入时，已完成的源文件在计划阶段直接跳过（分诊为「已导入」）
```
...
- **重复检测**：左栏底部「查找近重复照片…」→ 阈值档位 → 检测（后台 dHash + 汉明距离贪心聚类，
  200ms 进度 + 取消；作用域 = 当前目录全部照片，**同 stem 多格式组（RAW+JPEG）视为同一张不参与**）→
  分组卡片（首张 = 保留锚点，带「保留」徽标）→ 两个动作：**「其余标 Rejected」**（非破坏性，走旗标链路，
  网格/筛选即时生效）与 **「保留首张，其余移入回收站」**（二次确认；走 `delete_paths`，可先标 Rejected 复查）。
  结果与阈值落 `folder_db`（`.pt/data.db` 的 `duplicates` 表），重开弹窗直接读回，不重算。
...
5. ~~框选识别只画框，尚未接识别动作。~~ **已于 2026-09-22 接线**：预览工具条「框选」toggle
   （scan 图标）→ 拖拽画框 → 松开即识别（跳过检测直接分类），结果作为新主体**追加**进该照片
   已有 subjects（漏检补充），落 folder_db + 全局索引（冒烟 `region_smoke`）。

### 实现层缺口

6. 面板宽度双轨（本地存储 vs 配置字段），不会跨设备同步。
7. 导出/导入目标目录互不记忆。
8. ✅ 重复检测的阈值与结果都不落盘，重启后需重算。（2026-09-22 关闭：结果与阈值落 `folder_db`
   的 `duplicates` 表，重开弹窗读回；`duplicates_smoke` 覆盖「重开读回」）
