...
pub use raw_processor::{DecodeOptions, ImageFormat, RawError, RawProcessor, ThumbnailData};
...
/// Convenience function to extract RAW image with half_size demosaic (4x faster)
///
/// half_size=1 → output resolution halved, demosaic ~4x faster.
/// Ideal for preview of high-resolution RAW files.
pub fn extract_image_half_size<P: AsRef<std::path::Path>>(
    path: P,
    half_size: bool,
) -> Result<ThumbnailData, RawError> {
    RawProcessor::extract_image_half_size(path, half_size)
}

/// Convenience function to extract RAW image with custom decode options
///
/// Same pipeline as `extract_image`, but applies `opts` (demosaic quality,
/// output bit depth, half_size, auto-brightness) before unpacking.
/// Use `DecodeOptions::preview()` for the fastest decode (expected 4-8x).
///
/// # Example
///
/// ```no_run
...
///
/// let img = extract_image_with_options("photo.cr2", &DecodeOptions::preview()).unwrap();
/// ```
pub fn extract_image_with_options<P: AsRef<std::path::Path>>(
    path: P,
    opts: &DecodeOptions,
) -> Result<ThumbnailData, RawError> {
    RawProcessor::extract_image_with_options(path, opts)
}
