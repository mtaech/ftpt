...
│   ├── parallel_processing.rs # 并行处理示例（缩略图 + 完整解码）
│   ├── bench_decode.rs        # 完整解码管线性能基准（阶段计时 + 并行吞吐）
│   └── decode_debug.rs        # 白平衡/偏色诊断工具（缩略图基准 vs 解码输出对比）
├── libraw/
│   ├── msvc/      # Windows MSVC 预编译静态库 + 头文件
│   └── gnu/       # Linux x86_64 预编译静态库（ELF，含依赖）+ 头文件
...
3. 回退到 `libraw/gnu/lib/` 打包的静态库（`libraw.a`，当前为 LibRaw 0.22.2 的 Linux x86_64 ELF 全静态构建）
   - 该构建启用了 OpenMP，且为全静态：需依次链接打包目录内的 `libjpeg.a`、`liblcms2.a`、`libz.a`、`libgomp.a`（build.rs 已自动处理，依赖项按顺序放在 libraw 之后）
4. 所有路径均需链接 `libstdc++`

**Windows MSVC：**
- 直接使用 `libraw/msvc/lib/libraw_static.lib` 静态链接（当前为 LibRaw 0.22.2，编译时已启用 OpenMP 多线程和 AVX2 指令集）

**Windows GNU（MinGW）：**
- 使用 `libraw/gnu/lib/libraw.a` 静态链接（需 `-fPIC` 编译）
- 注意：0.22.2 起 bundled gnu 库为 Linux ELF 格式，**不能用于 MinGW 链接**（build.rs 会发出 cargo warning），windows-gnu 目标需自行用 MinGW 编译 libraw
- 回退到动态链接（`libraw.dll`）
- 额外链接 `libstdc++`
...
```rust
// raw_processor 模块
pub use raw_processor::{RawProcessor, ThumbnailData, ImageFormat, RawError, DecodeOptions};
...
pub fn extract_image_with_options(path, opts: &DecodeOptions) -> Result<ThumbnailData, RawError>;
```
...
impl RawProcessor {
    ...
    pub fn set_decode_options(&mut self, opts: &DecodeOptions);  // 设置解码参数（unpack 前调用）
    pub fn extract_image_with_options<P: AsRef<Path>>(path: P, opts: &DecodeOptions) -> Result<ThumbnailData>; // 按参数一键完整解码
    pub fn version() -> String;                                  // 版本字符串
    pub fn version_number() -> i32;                              // 整数版本号
}
```

#### `DecodeOptions` — 完整解码参数

控制 `dcraw_process()` 的画质/速度权衡，所有字段须在 `unpack()` 之前设置才生效。

```rust
pub struct DecodeOptions {
    pub half_size: bool,       // 半尺寸去马赛克（~4x 加速）
    pub demosaic_quality: i32, // 0=bilinear(最快) 1=VNG 2=PPG 3=AHD(默认)
    pub output_bps: i32,       // 8 或 16，8 位输出内存减半
    pub no_auto_bright: bool,  // 跳过自动亮度直方图扫描
    pub output_color: i32,     // 0=RAW(跳过色彩转换) 1=sRGB(默认)
    pub linear_gamma: bool,    // 线性输出（跳过 gamma 曲线，自建色彩管线用）
    pub use_camera_wb: bool,   // 相机白平衡；LibRaw 原生默认关闭会偏色，两个预设均开启
}

impl DecodeOptions {
    pub fn quality() -> Self;  // 画质优先（AHD+16bit+自动亮度+相机白平衡）
    pub fn preview() -> Self;  // 快速预览（half+bilinear+8bit+no_auto_bright+相机白平衡）
}
```

注意：`extract_image()` / `extract_image_half_size()` 及两个预设均已默认开启相机白平衡（0.6.x 起，修复 LibRaw 默认日光白平衡导致的偏黄问题）；如需 LibRaw 原生行为，用 `RawProcessor` 手动走管线且不要调用 `set_use_camera_wb`。
...
impl ParallelProcessor {
    ...
    // 完整解码并行（非缩略图）：每线程复用一个 RawProcessor（recycle 重置），
    // opts 控制解码参数，如 DecodeOptions::preview()；
    // jobs>1 且未显式设置 OMP_NUM_THREADS 时会自动设为 1（避免与 LibRaw 内部 OpenMP 超售，
    // 实测吞吐差 15-20%；注意 OpenMP 首次初始化后环境变量不再生效）
    pub fn process_images<P>(files: &[P], config: &ParallelConfig, opts: &DecodeOptions) -> Vec<ProcessResult>;
}
```
