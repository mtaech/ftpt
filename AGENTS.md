...
- **Linux「复制图片」别再用 arboard 的 `set_image`**（2026-09-25）：它的公开 API 只有 `set_image`
  （RGBA → **同步 PNG 编码**）、`set_file_list`、文本 / HTML，没有「按任意 MIME 塞原始字节」的入口；
  39MP 照片就是秒级 CPU + 几百 MB 内存，而且（第一版实现）这一步曾在**主线程**做——直接冻住 UI
  （实测 3000×2000 噪声图 3867ms）。现在统一走 `state/clipboard_file.rs`：Wayland 用
  `wl-clipboard-rs::copy::copy_multi`（多 MIME、库内部线程服务、**不 fork**），X11 用 `x11rb` 自写
  selection 服务器（`x11-clipboard` 只支持单目标、arboard 不暴露它的 `write`）。只有回退路径
  （RAW / 平台不支持）才用 arboard，且必须在**后台线程**调用。诊断开关 `PHOTO_CLIPBOARD_LEGACY_PNG=1`。
- **cargo 默认吃满全部核心，会打断前台工作**（2026-09-24）：`.cargo/config.toml` 里加了
  `[build] jobs = 8`（16 核留一半给前台）与 `rustc-wrapper = "scripts/nice-rustc.sh"`——
  包装脚本把 rustc **及其派生的链接器**降到 `nice -n 19` / `ionice -c 2 -n 7`（best-effort 最低档），
  前台一旦要 CPU/磁盘就优先拿到；缺 nice/ionice 时脚本降级为直接 exec，不会让构建失败。
  验证手法：`cargo check -v` 的输出里应出现 `scripts/nice-rustc.sh <rustc> …`；
  想全速构建时 `cargo build -j 16`。**别把 `jobs` 调回默认**——用户明确要求「编译时别卡我」。
...
- Rust：`#[test]` 分布在 5 个 crate 的源文件内联 `#[cfg(test)] mod tests`（domain 39 / config 23 / engine 211 / recognize 35 / photo-ui 120），另有 1 个 `#[ignore]` 真机冒烟
...
|`photo-recognize`|35 + 1 ignore|阶段→状态映射、输入源解析（JPEG/RAW）、org_det 输出解码（布局/近满幅让位/越界夹紧）、BioCLIP 后端（npy 解析/species_of/中文名三级回落/top-k 检索/**地区过滤子矩阵裁剪**）、按学名查名录、进度回调|
...
- **图片**：`image/`（`ImageManager` 进程内解码 + 缓存：缩略图 → 2560 显示母版 → 1:1 全分辨率）；**复制到剪贴板**（Ctrl+C / 预览工具条「复制」）在 `state/engine_ops.rs`：常规格式走 `state/clipboard_file.rs` 把**源文件原字节**按它自己的 MIME + `text/uri-list`（文件路径）挂上剪贴板（不解码、不重编码；Wayland = `wl-clipboard-rs` 多 MIME，X11 = 自写 selection 服务器），RAW / 平台不支持时回退「解码全尺寸 RGBA → arboard PNG」——**两条路径都在后台线程**（GPUI 自带的剪贴板在 Linux 只写文本，图片项会被静默丢弃）
...
- **2026-09-25 fix(photo-ui)：切到单张后底部胶片条没跟着定位（滚动条停在最左）**：
  用户报「切换到单张的时候底下的滚动条没有同步定位」——单张态底部胶片条一直停在第 0 张，当前那张在
  视口外，得手动拖滚动条才找得到自己。
  **修法**：① 把胶片条几何（项宽 96 / 间距 6 / padding 8 + 内容宽公式）从视图里抽到 `model/filmstrip.rs`
  ——「滚动到当前那张」要按同一套公式反推偏移，两处各写一份必然漂移；② 纯函数
  `ensure_visible_offset`：已经在可视区就返回 `None`（不动），否则**居中**并夹到 `max_offset`；
  ③ `AppState::sync_filmstrip_to_active(key)` 以「扫描世代 + 主选中项」为键，**只在换了图之后做一次**
  ——用户手动拖动胶片条时主选中项没变，不会被每帧拽回；④ 视口尺寸要等胶片条布局过一次才有，所以调用
  放在 `views/filmstrip` 的 `defer_in` 里（与筛选栏同步候选同一手法），改完偏移再 `cx.notify()`。
  渲染期只有 `&AppState`，而 `ScrollHandle` 本身是 `Rc<RefCell<_>>`，所以同步进度用 `Cell` 记。
  **验证（先红后绿）**：`stats_smoke` 15→**17 项**——预览态 16 张里选最后一张：红跑（关掉同步）
  实测偏移停在 `0`、当前项在内容坐标 1538 处（视口宽 780）看不见；恢复后偏移 `-862`（滚到内容末端，
  当前项进可视区），并且「没换图时偏移不再变动」也过。`cargo test -p photo-ui` 116→**120**
  （胶片条几何 + 定位四条单测）；`cargo check --workspace --all-targets` 0 warning；
  `lease_smoke` / `context_menu_smoke` / `a11y_smoke` 回归全过。

- **2026-09-25 fix(photo-ui)：点击「最近打开」后界面是空的 —— 顶层没照片时自动包含子目录**：
  用户报「我点击最近打开后没有自动加载数据，空的」。**根因**：卡上的源目录根下只有 `DCIM/`，照片都在
  子目录里，当初是用「仅添加并浏览」（**显式递归**）打开的；而之后点「最近打开」那一行重开时，左栏目录行
  的点击一律按 `AppConfig.include_subdirectories`（默认**单层**）扫 → 顶层 0 张 → 界面全空（文件其实都在
  盘上）。同一路径也解释了「启动恢复上次目录」为什么可能是空的（`last_directory` 从 2026-09-25 起才真的会写）。
  **修法**：`start_scan` 完成后，如果**这次是打开另一个目录**（不是重扫当前视图）且顶层一张照片都没有，
  就自动改成包含子目录再扫一次，并在状态栏说明（随后由收尾的「扫描完成，共 N 张」接上）。判据里的
  「换了目录」（`previous_dir != dir`）是刻意的：F5 / 删除 / 撤销这类**重扫当前视图**不动，否则删掉顶层
  最后一张后会悄悄把视图改宽（`rescan_smoke` 第 8 段钉的就是这个语义）。自愈后的视图模式是递归，
  所以后续 F5 不会再变空。
  **验证（先红后绿）**：`rescan_smoke` 12→**15 项**——第 9 段建 `card/DCIM/100_CANON/card.jpg` 后打开
  `card/`（单层）：红跑把自愈关掉，实测列表是 `[]`、3 项失败（正是用户看到的「空的」）；恢复后 15/15 全过
  （含「F5 之后卡里的照片还在」）。`cargo check --workspace --all-targets` 0 warning；`cargo test -p photo-ui`
  **116 项**；`folder_picker_smoke` / `stats_smoke` / `context_menu_smoke` / `batch_ops_smoke` 回归全过。

- **2026-09-25 fix(photo-ui)：复制图片不再卡死，并改成「源文件原字节 + 文件路径」**：
  用户报「复制图片会导致卡死」。**根因**：arboard 在 Linux 上的 `set_image`（X11 与 Wayland 两条路径都是）
  会**同步把 RGBA 编码成 PNG**，而这一步以前写在 `async_cx.update` 里（**主线程**）——3000×2000 噪声图实测
  独占 UI 线程 **3867ms**，39MP 只会更久。
  **第一刀**：解码与写剪贴板一起挪进后台 executor，前台只更新状态栏（并先给一条「正在复制…」）。
  **第二刀（用户拍板「直接把源图片复制进剪贴板」）**：不再重编码，新增 `state/clipboard_file.rs`：
  - **Wayland**：`wl-clipboard-rs`（arboard 的底层依赖）的 `copy_multi` 一次挂 `image/jpeg`
    （**源文件原字节**）+ `text/uri-list`（文件路径）+ 自动附带的 UTF8 文本；库内部起线程服务、立即返回、
    出错同步返回（**不 fork**，避免多线程进程里 fork 的坑）；
  - **X11**：自己写最小 selection 服务器（`x11rb`，同样已在依赖树里），应答 `TARGETS` / `TIMESTAMP` /
    图片 MIME / `text/uri-list` / `UTF8_STRING`；数据由常驻线程持有，新一次复制顶掉旧的（旧线程收
    `SelectionClear` 后退出）。
  RAW（没有可直接粘贴的 MIME）与「原字节路径失败」时**回退**老的「解码 → arboard PNG」；回退也可用
  `PHOTO_CLIPBOARD_LEGACY_PNG=1`（或 `clipboard_file::set_legacy_png`）强制触发，供冒烟 / 诊断。
  状态栏区分两条路径（「已复制 X（image/jpeg 原文件 + 文件路径，未重编码）」vs「已复制图片到剪贴板…」）。
  **验证**：
  - `clipboard_smoke` 重做：小 PNG 仍能被 arboard 读回 64×48；大 JPEG 断言 `TARGETS` 含 `image/jpeg` +
    `text/uri-list`、**`image/jpeg` 与源文件逐字节相同**（7514371 字节，零重编码）、`text/uri-list` 等于
    `clipboard_file::uri_list`；再强制回退 PNG 路径，断言 arboard 读回 3000×2000，且**两条路径期间 UI 最大
    停顿都 < 400ms**（修前 3867ms）。冒烟里为此加了 x11rb 读剪贴板的小工具。
  - **Wayland 分支单独在隔离的嵌套合成器里验过**（`kwin_wayland --virtual --socket wayland-probe`，
    不碰用户会话）：`wl-paste --list-types` = image/jpeg + text/uri-list + 文本那几个；
    `wl-paste --type image/jpeg` 与源文件 `cmp` **逐字节相同**（676917 字节）；`text/uri-list` 是绝对路径。
  - `cargo test -p photo-ui` 113→**116**（MIME 映射、URI 百分号编码）；`cargo check --workspace --all-targets`
    0 warning；`context_menu_smoke` / `lease_smoke` 回归全过。
  **范围**：手写 selection 服务器只覆盖 Linux；Windows / macOS 继续走 arboard 的 PNG 路径
  （`copy_file_as_image` 在这些平台直接返回 Err，调用方回退）。

- **2026-09-25 feat(photo-ui)：左栏目录行补上「收藏 / 取消收藏」与「从最近打开移除」**：
  用户报「最近打开不能移除和收藏」。查下来这两个动作**在代码里根本不存在**：`favorite_dirs` 只被读来渲染
  （没有任何增删入口），`recent_dirs` 连移除都没有——上一轮刚把「最近打开 / 上次目录」接通落盘，但列表本身
  仍然是只增不减的死数据。
  **做法**：① `AppState::toggle_favorite_dir`（星标开关，返回切换后的状态）与 `remove_recent_dir`
  （只动列表，不碰磁盘），两个都同步 `AppConfig` 并 `save_config()`；移除的目录正好是「上次打开的目录」时，
  `last_directory` 一起退到新的第一条（否则下次启动又把它打开、再塞回列表）。② 左栏目录行加行尾操作：
  所有目录行有星标（实心 = 已收藏），「最近打开」另有 ×；星标 / × 是行的**子元素**，GPUI 命中取最上面的
  元素，所以点它们不会同时触发整行的「打开这个目录」。行 hover 补完整路径 tooltip。
  **顺带修排版**：父目录名上限 76→48px、图标内边距收紧——加两个按钮后主名字曾被挤到 ~36px（只剩
  「2026-...」），现在窄面板下也读得出「100_FUJI」这种短名。
  **验证**：`folder_picker_smoke` 7→**12 项**（星标加入收藏 → 再点取消收藏 → × 移除最近 → 运行时 /
  AppConfig / **重读配置文件**三处都掉，且移除最近不影响已收藏）；截图目检左栏三种行（子目录 / 收藏 /
  最近打开）；`cargo test -p photo-ui` **113 项**、`cargo check --workspace --all-targets` 0 warning、
  `lease_smoke`（左栏）回归全过。
...
- **2026-09-24 feat(photo-recognize/photo-config/photo-ui)：地区设置 —— 识别候选按「省级鸟种分布」裁剪（todo #19）**：
  ...
  做法：`AppConfig.recognition_region`（空串 = 全国）+ `Recognizer::with_region` 在装配时把 top-k
  检索裁剪到「该省有 GBIF 出现记录」的鸟种（非鸟标签与无分布数据的鸟种恒不过滤），设置页
  「识别与性能」加可搜索下拉 + 状态栏「地区: X」常驻标记。数据由 `bioclip_demo/data/build_bird_regions.py`
  用 GBIF 的 `facet=stateProvince&facetLimit=200` 单查询（~1s/种）生成 `data/taxon/bird_regions.json`
  （可选资产，1356 种 / 14678 条映射）。真照片对照：全国/浙江 top-1 都是东方白鹳 76.4%，
  陕西（该鹳 0 条记录）则把它排除、top-1 换成钳嘴鹳 71.8%。**踩坑见 todo #19**（子矩阵搬运
  把 `[dim, cols]` 行优先当列连续抄，单元测试用了同样的错布局所以是绿的——真照片验证才抓到）。
...
- **2026-09-24 feat(photo-ui/photo-recognize)：把 todo 剩下的四项做完（#13.3 / #13.4 / #17 / #16）**：
  ...
  再做机械的小件，最后做必须真模型验证的识别增强。
  **① #13.3 镜头 / 物种多选筛选器**（原条目写「无方向键导航」，实际 GPUI 版**连控件都没有**）：
  `model/filter.rs` 加 `lens_options` / `taxon_options`（去重排序；物种候选必须含
  **各主体展示名**——筛选语义是「任一主体命中即保留」，只列顶层会让多主体照片的次要物种选不到）；
  AppState 持两个 gpui-component **Combobox**（多选 + 可搜索 + 自带方向键），Change/Confirm 写回
  criteria；候选随扫描世代**在渲染期经 `defer_in` 重建**（`set_items` 需要 Window，而扫描完成
  那一刻没有 window——这是本条的坑，见下）。筛选栏加两行 + 折叠行可清除 chip（清除要同时清
  criteria 与下拉选中集，否则下拉还勾着、再点一下筛选就回来了）。验证：`filter_bar_smoke`
  8→**17 项**（含渲染期自动同步、多值 = 命中任一）。
  **② #13.4 照片右键菜单**（原条目抱怨「无键盘导航」，实际**连菜单都没有**；而 gpui-component 的
  PopupMenu 全模块没有一处键盘处理，套用等于把缺口原样搬过来 → 自绘）：纯逻辑
  `model/context_menu.rs`（10 项、初始高亮、上下移动**跳过置灰项且到边界不环绕**）+
  `views/context_menu.rs`（背景层点空白关 + 卡片 240px 最大高、超出走 `scroll_area_v` 的
  track_scroll + Scrollbar、贴边往回收）+ 根视图 `on_key_down`（Up/Down/Enter；根视图持有焦点，
  菜单不抢焦点）+ `handle_escape` 第 0 优先级；`engine_ops::apply_context_menu_action` 一条分发
  服务点击与回车（打开预览 / 复制图片 / 复制路径 / 打开所在文件夹 / Pick / Reject / 清旗标 /
  评 5 星 / 识别这一张 / 移至回收站只弹确认框），目标已不在目录时诚实报错。网格 cell、胶片条、
  预览三处接上右键。验证：新增 `context_menu_smoke` **20 项**——含「10 项 × 28px 超过 240px
  最大高 → `max_offset` 真的 50px」这条（不是「有滚动条但滚不动」）。
  **③ #17 识别器空闲自动卸载**：`model/recognizer.rs::should_unload_recognizer` 纯判据
  （配置 0=关闭 / 未装配 / **识别进行中绝不释放** / 空闲未满阈值）+ `AppConfig.recognizer_idle_unload_minutes`
  （默认 **0 = 关闭**）+ `start_recognizer_idle_watch`（30s 一次；`PHOTO_RECOGNIZER_IDLE_TICK_MS`
  仅供冒烟压到秒级）+ 设置页 关闭/5/15/30 档位。`recognizer_last_used` 在识别起点与「识别进行中」
  的每个 tick 都刷新——否则「空闲很久 → 跑一个 5 秒的批量」会在批量刚结束时被判成空闲超时。
  验证：`region_smoke` 16→**19 项**，用**真实模型装配后**验证 busy 保护与自动释放。
  **④ #16 org_det 多框交叉验证**：`detect::class_is_consistent`（19 粗类 → 界/门/纲，规则逐条对齐
  `../bioclip_demo/src/detect.rs:186`）+ `pipeline::cross_validate_subjects`（分类后过滤，
  全丢时**退回整图识别**、index 重排连续、逐框 debug 日志）。
  **与 demo 的有意差别**：本仓名录兜底路径的 `TaxonMatch.ranks` 是空 vec（`catalog.rs:54`），
  demo 那边路径永远来自标签空间 → 没有分类证据时**一律放行**，否则走名录兜底的主体（置信度不低的
  那批）会被整批误判成误检。**位置更正**：`bioclip_demo` 不在本仓库，是兄弟目录 `../bioclip_demo`。
  验证：`cargo test -p photo-recognize` 27→**33**；真照片 `crates/rawlib/img.jpg` →
  东方白鹳 76.4% 无回归；`recognize_smoke` 5/5、`region_smoke` 19/19。
  **未复现到「误检框被丢」的真机场景**（org_det 不再有旧单类鸟检测器那种把花判成鸟的假框），
  该路径由 3 个单测覆盖——如实写在这里，不当成「已验证」。
  **踩坑（留给以后）**：① 需要 Window 的实体初始化（Combobox/输入框）只能放在 `AppState::build`
  或渲染期 `defer_in`，扫描完成那条回调里没有 window；② 无头冒烟里改 items 之后**必须
  `cx.notify()`**，否则渲染驱动的候选同步与滚动范围断言都不会发生（本轮 filter_bar_smoke
  第一次就踩了这个，第二处是「等 !is_scanning」在重扫开始前就通过）；③ 自绘菜单的「点空白关闭」
  用「背景层 + 卡片作后一个兄弟」实现（GPUI 命中取最上面的元素，不需要 stop_propagation）。

- **2026-09-24 feat(photo-ui/photo-engine)：批量操作整条链路接线 + 删除可撤销 + 一批 P3 打磨（todo #13/#14/#17/#1）**：
  起因是用户要求「把 `docs/todo.md` 里没做完的挨个做完」，做 #13.1/#13.2 时**发现条目本身写轻了**：
  GPUI 版左栏「复制到目录 / 移动到目录」弹完确认框**什么都不做**、批量重命名**连入口都没有**，
  而 `op_journal` 全仓从未 `record`——Ctrl+Z 永远回答「没有可撤销的批量操作」（手册承诺的撤销从未生效）。
  引擎侧 `batch_ops::execute` / `ops::rename_captures_templated` 早已就绪并带单测，又是「链路断在 UI」。
  **做法**：① **引擎**——`ops::delete_file_to_trash` 删除后回查回收站条目（`locate_trashed`：删除前后
  `os_limited::list()` 差集 + canonical 原路径匹配）→ 新 `undo::TrashedItem` / `UndoOp::Trash`，
  `restore_from_trash` 走 freedesktop/Windows 回收站 API 真恢复（macOS 报可读原因，不留半吊子）；
  `batch_ops::execute_journaled` 逐文件执行并产出撤销记录（旧 `execute` 保留为薄包装）；
  `ops::rename_captures_templated_journaled` 同理（同名不改名不记日志）。② **UI**——`engine_ops`
  新增 `start_batch_op` / `start_batch_rename`（重扫取作用域 → 后台执行 → 记 journal → 重扫）、
  `release_recognizer`、`pick_batch_target_dir`；Delete 键改为先弹 `ActiveDialog::DeleteConfirm`；
  新增 `Rename` 弹窗（模板 + 实时预览，走 `model/export` 同款纯逻辑）；Ctrl+Z 报成功/跳过原因、
  回收站恢复单独播报；左栏批量面板加 **常驻结果行**（状态栏那条会被随后的「扫描完成…」顶掉，
  用户看不到「Ctrl+Z 可撤销」）。③ **#13.1 逃生门**——无筛选时按钮仍默认禁用，多一个「我确认，
  对全目录 N 项执行」；**解锁状态任何筛选变化自动收回**（`recompute_pipeline`）。④ **#13.5**——
  `CaptureMeta` 新增 `failure_stage`/`candidates`（domain，Top-3），信息栏显示失败原因中文与
  「最接近：A 61.5% · B 55.0%」，多主体行改「相似度 68.3%」。⑤ **#14.7** 导入目标目录记忆
  （`AppConfig.import_dir`）。⑥ **#1 遗留** 导出预设新建/保存/删除（`model/export.rs` 的
  `preset_from_draft`/`preset_index_for_draft` + 弹窗按钮 + 单元测试）。⑦ **#17** 设置页
  「释放识别器」按钮。⑧ **构建资源**——`.cargo/config.toml` 加 `[build] jobs = 8` 与
  ...
  **验证**：`cargo check --workspace --all-targets` 0 warning；`cargo test` 全绿
  （domain 35→**38**、config 18→**19**、engine 191→**199**、photo-ui 81→**85**）；
  新增无头冒烟 **`batch_ops_smoke` 25 项 ALL PASS**（含「确认后进回收站 → Ctrl+Z 真恢复」、
  移动/复制/重命名各自撤销）；`lease_smoke` 15 项、`export_smoke` 27 项回归全过。
  **顺带核实**：#14.6「面板宽度双轨」在 GPUI 版**不成立**（宽度只在 `AppConfig`，无第二条本地存储轨）；
  #13.3/#13.4 的「左栏镜头/物种多选下拉」「右键菜单」在 GPUI 版**根本没有这两个控件**（Tauri 版条目），
  要做等于新建功能，已在 todo 里标注。
  **冒烟里踩的坑（留给以后）**：① 撤销/批量操作后会 defer 一次重扫，「等 `!is_scanning`」可能在重扫
  **开始之前**就通过（`display_order` 还是旧的/空的）→ 断言必须等「扫描结束 **且** 可见集恢复到 N 张」；
  ② 批量操作的结果消息会被收尾重扫的「扫描完成，共 N 张」覆盖，所以断言/展示都改用常驻字段。
...
- **2026-09-23 chore(发布)：补 `NOTICE` + 修 `scripts/package.ps1`（此前根本不拷 `data/taxon/`）——顺手查出 AGPL 阻塞项**（`docs/todo.md` #15 · `60b1a57` + AGPL 边界 `a8e506b`）：
  ...
  **⚠️ 核定中查出的新问题（已定，非阻塞）**：`models/org_det.onnx`（ADR 0010 换上的通用检测器）基于 **Ultralytics YOLOE-26s-seg**，而 Ultralytics 代码/权重是 **AGPL-3.0**（README 原文：AGPL-3.0 或申请 Enterprise License）。**更正一下我自己的说法**：当时写成「发布前唯一的合规阻塞项」并归到「闭源分发才有问题」——不确切。**AGPL 是 copyleft，不是「非商业」限制，本仓不收费并不豁免它**；它真正要求的是分发时① 整个组合作品按 AGPL-3.0 授权、② 提供完整对应源码、③ 附许可文本。本仓代码公开且不收费，三条本就满足，所以取定：**含 org_det.onnx 的发布包整体按 AGPL-3.0 分发，仓库自身文件仍 MIT**（MIT 可被并入 AGPL 作品），已新增 `LICENSE-AGPL-3.0.txt`（GNU 原文）并由 `package.ps1` 随包收集；唯一实质影响是别人不能把你这份代码连同该权重搬进闭源产品。要彻底避开就换许可宽松的检测模型（Apache-2.0 家系的 RT-DETR / YOLOX 类），要简单就把整仓改 AGPL。
  ② **`scripts/package.ps1`（真 bug）**：原来只拷 `models/*.onnx` 与 `data/bird_catalog.db`——**`data/taxon/` 根本没进包**（识别器会在 `data_root()/data/taxon/` 找不到资产直接报 `ModelLoad`），本次加进必需资产 + 四个必需文件 + `VERSION` 的 `schema`/`dim`/`labels` 字段校验，并把 `NOTICE` / `LICENSE` / `LICENSE-AGPL-3.0.txt` 随包。zip 环节从 `Compress-Archive` 换成系统自带 bsdtar（`tar -a -c -f`）：理由① 前者对 2GB+ 有已知上限，② 包里的 `.onnx`/`.npy` 接近随机字节，deflate 基本压不动、只在浪费时间；加了 **bsdtar 身份校验**（`tar --version` 必须含 bsdtar）——PATH 前面若是 Git/MSYS 的 GNU tar，`-a` 不认 zip 会**静默产出 tar 文件**，宁可直接报错；调用改为 `Push-Location dist` 后用相对路径，避开 tar 对 `C:\` 盘符参数的处理差异。
  **验证与局限**：`cargo test` / `cargo check --workspace --all-targets` 仍全绿（本次不动 Rust）；`data/taxon/VERSION` 确含 `schema:1/dim:768/labels:93452`；bsdtar 命令形状在本地核过；但**开发机是 Linux、没有 pwsh，`package.ps1` 没法端到端跑**——这点已写进 todo 的「未验证部分」，别把它当已验证。

- **2026-09-23 feat(photo-ui/photo-domain)：语义收敛 —— 识别卡改「相似度 + top1−top2 间隔」、中文名缺失回落「属中文名 + 学名」**（`docs/todo.md` #5 / #6，Batch 4 · `9ec8a08`）：两件拍板后一直挂着的小事。
  ① **不再叫「置信度」**：BioCLIP 的 confidence 是**余弦相似度 × 100**，实测 top-1 落在 65–79 窄带、**跨物种不可比**（0.75 对鸟算高，对植物算什么不知道），叫「置信度」会被当成 softmax 概率用。信息栏从裸百分数改为 **`相似度 68.3% · 间隔 12.1`**（手册 §9.7 同步）；**不上硬拒识阈值**（实测纯噪声图 0.711 > 真实照片最小值 0.645，没有干净阈值），改用**相对判据**：`photo_domain::gap_to_runner_up(top1, candidates)` = top1 − top2（`candidates` 在 classifier 里已 `skip(1)`，首位就是第二名——这条口径写进了函数文档，改动前先核对过 `bioclip.rs`），落成 `CaptureMeta.taxon_gap`，和相似度同处展示。
  ② **中文名三级回落带上学名**：`TaxonMatch::display_name()` 从「有中文名就用、否则学名」改成按 `cn_level` 分口径——种级直接用；**只有属/科级（种级缺失）→ 「属中文名 + 学名」**（`粉褶蕈属 Entoloma nitidum`，单看属名会被当成种名）；三级都没有才只显示学名。与 bioclip_demo `data/README.md` 的 UX 一致（该文件原话：「这是这类应用正确的 UX，不是缺陷」）。返回值 `&str → String`（要拼接），6 个调用点、3 个测试字面量同步。
  **验证（先加测试再改）**：`cargo check --workspace --all-targets` 0 warning；`cargo test` 全绿（domain 35→**37**：新增 `test_display_name_falls_back_by_cn_level` 种/属/科/缺四档、`test_gap_to_runner_up` 含空候选/无 Top-1 边界；photo-ui 81 不变但补了「回退主体的间隔也要带回」断言——顶层无结论时别只回填相似度）。
  **顺带记录**：手册 §9.7 写的「失败阶段中文 + 最接近候选」仍未接线（改了手册时发现并标注，避免下次再当成已实现）；信息栏的多主体行仍只显示百分数。
