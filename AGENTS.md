...
|无头冒烟|`XDG_CONFIG_HOME=/tmp/ptlease-config xvfb-run -a cargo run -p photo-ui --example lease_smoke`（另有 `preview_full_smoke`、`clipboard_smoke`、`adjust_smoke`、`grid_scroll_smoke`、`filter_bar_smoke`、`recognize_smoke`、`region_smoke`、`master_bench`）|
...
- **无头冒烟必须钉住 X11 后端**：GPUI 选后端只看环境变量（`platform::guess_compositor()`）：`WAYLAND_DISPLAY` 非空 → Wayland，其次 `DISPLAY` → X11。而 `xvfb-run` 只准备 X 显示：在 Wayland 会话里跑冒烟时窗口会落到**用户真实桌面**，帧由真实合成器决定（被遮挡/最小化时不产帧），于是**渲染驱动的行为根本没被测**——渲染驱动的检查（如「对比窗格加载母版」）就因此同一份代码一会儿 13/13 一会儿 12/13（母版加载只在视图真的渲染时才发起；`compare_smoke` 当年就是这样抓出来的，该视图已移除）。所以 8 个 GPUI 冒烟都在 `gpui_kit::application()` 之前调 `photo_ui::app::prepare_headless_smoke()`（把 `WAYLAND_DISPLAY` 置空，GPUI 判空即视为未设置）；要真机 Wayland 目检时设 `PHOTO_SMOKE_ALLOW_WAYLAND=1`。
...
- Rust：`#[test]` 分布在 5 个 crate 的源文件内联 `#[cfg(test)] mod tests`（domain 34 / config 18 / engine 185 / recognize 27 / photo-ui 60），另有 1 个 `#[ignore]` 真机冒烟
...
|`photo-ui`|60|筛选/排序/堆叠/连拍/预览数学/调整参数（量化、chip 文案、相对路径、字段操作）纯逻辑（`src/model/`）、Material You 色彩（`theme/`）、剪贴板解码|
...
- **2026-09-22 perf(识别)：识别器常驻缓存（懒装配一次，批量/框选共用）**：修掉「每次框选识别都重装一遍模型」。① **photo-recognize**：`Classifier` trait 加 `Send` 超约束（唯一实现 `BioClipClassifier` 持有 ort `Session` + ndarray，均 Send；`CatalogDb` 的 rusqlite `Connection` 本就 Send），使 `Recognizer: Send` 可跨后台线程；补编译期断言单测 `test_recognizer_is_send`。② **photo-ui**：`AppState.recognizer: SharedRecognizer`（`Arc<parking_lot::Mutex<Option<Recognizer>>>`，`AppState::new` 初始化为 None = 未装配）；新增 `engine_ops::ensure_recognizer(slot, models_dir, catalog_db)`——已装配直接返回，否则装配并缓存（**失败不缓存**，下次重试）。③ **接线**：批量识别（`start_recognition`）与框选识别（`start_region_recognition`）都从 `state.recognizer.clone()` 取同一槽位；**锁只在单张推理期间持有**（不整批持有），两条路径互斥但可在照片之间交错；装配日志只在首次打印。
  **验证**：`cargo check --workspace --all-targets` 0 warning；`cargo test` 全绿（photo-recognize 26→27）；`region_smoke` 重写为「冷/热两段」11 项全过——实测 **首次框选 3.07s（含模型装配）→ 第二次框选 1.87s（复用常驻），省 ~1.2s 装配**，槽位 None→Some 并保持，批量识别复用同一实例；`recognize_smoke` 5/5。**附带 A/B 结论（未采纳）**：给 `photo-recognize`/`ndarray` 加 debug `opt-level=3` 实测**无收益**（热复用 1.87s vs 1.82s，噪声内）——瓶颈在 ONNX Runtime 的 C 库而非 Rust ndarray 侧，故**未改** Cargo.toml（不做无依据的改动）。**代价**：模型常驻内存（org_det 40MB + BioCLIP int8 293MB + 名录 embedding 287MB，约 600MB+ 长期占用；此前识别完即释放）。
